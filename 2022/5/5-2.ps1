...
for ($i=0;$i -lt $inputdata.Count;$i++){
    if ($inputdata[$i]) {
        ...
        for ($j=1;$j -lt $line.Count;$j=$j+4){
            if ($line[$j] -in "A".."Z") {
                $crate = $line[$j]
                $stacks[$column].Add($crate) |out-null
            }
            $column++
        }
    } else {
...
for ($i=$endofstacks;$i -lt $inputdata.Count;$i++){
    ...
    [int]$source = $tokens[3]
    [int]$target = $tokens[5]
    $postomove = [math]::Max(($stacks[$source].count - $count),0)
    for ($j=1;$j -le $count;$j++){
        $stacks[$target].Add($stacks[$source][$postomove])|Out-Null
        $stacks[$source].RemoveAt($postomove) |Out-Null
    }
}
